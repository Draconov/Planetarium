...
Each release exposes one custom download: the portable Windows x64 executable generated by `electron-builder`. GitHub also adds its standard `Source code (zip)` and `Source code (tar.gz)` links automatically.
